...
namespace Collections.Isolated.Monads;

public record Result(Exception? Exception)
{
    public static ILogger? Logging { get; set; }
    ...
    {
        Logging?.LogWarning(exception.Message);
    ...
    public static Result<TContent> Failure<TContent>(string exceptionMessage, bool mute = false)
    {
        if(mute is false)
            Logging?.LogWarning(exceptionMessage);

        return new Result<TContent>(default, new Exception(exceptionMessage));
    }

    public static Result<TContent> TraceFailure<TContent>(string exceptionMessage, bool mute = false)
    {
        if (mute is false)
            Logging?.LogTrace(exceptionMessage);
